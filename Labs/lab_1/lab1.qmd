...
ggplot(county_data) +
  ...
  labs(
    title = "Medium Age vs Medium Income",
    x = "Medium Age",
    y = "Median Household Income ($)"
  ) +
  theme_dark() +
...
county_reliability <- county_data %>%
  mutate(
    med_age_moe_pct = (median_ageM / median_ageE) * 100,
    pop_category = case_when(
      total_popE < 50000 ~ "Small (<50K)",
      total_popE < 200000 ~ "Medium (50K-200K)",
      TRUE ~ "Large (200K+)"
...
# Box plots by population category
ggplot(county_reliability) +
  aes(x = pop_category, y = med_age_moe_pct, fill = pop_category) +
  geom_boxplot() +
