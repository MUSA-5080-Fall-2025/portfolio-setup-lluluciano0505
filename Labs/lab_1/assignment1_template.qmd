...
**Comment on the output:** I selected St. Louis, Buchanan, and Mississippi counties to represent high, moderate, and low data reliability contexts—urban, mid-sized, and rural areas, respectively. 
...
tract_demo <- tract_demo_raw %>%
  ...
  select(
    ...
    white_population = whiteE,
    black_population = blackE,
    hispanic_population = hispanicE,
    pct_white,
    pct_black,
    pct_hispanic
  )
...
white_avg    <- avg_moes$avg_white_moe
black_avg    <- avg_moes$avg_black_moe
hispanic_avg <- avg_moes$avg_hispanic_moe
...
tract_quality <- tract_demo %>%
  ...
  mutate(
    high_moe_flag = ifelse(
      coalesce(white_moe_pct    > 100, FALSE) |
      coalesce(black_moe_pct    > 100, FALSE) |
      coalesce(hispanic_moe_pct > 100, FALSE),
      TRUE, FALSE
    )
  )
